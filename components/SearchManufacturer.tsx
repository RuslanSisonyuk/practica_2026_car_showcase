...
const SearchManufacturer = ({ manufacturer, setManufacturer }: SearchManufacturerProps) => {
  ...
  return (
    <div className='search-manufacturer'>
        <Combobox
            value={manufacturer}
            // Accept both string and null for Combobox onChange
                ...
                >
                    <ComboboxOptions className='custom-filter__options'>
                        {filteredManufacturers.map((item) => (
                            <ComboboxOption key={item} className="search-manufacturer__option text-gray-900 data-[focus]:bg-primary-blue data-[focus]:text-white" value={item}>
                                {({ selected, focus }) => {
                                    return (
                                        <>
